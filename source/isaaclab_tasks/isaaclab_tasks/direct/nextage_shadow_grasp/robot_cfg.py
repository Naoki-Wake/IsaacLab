...
class RobotCfg():
    def __init__(self, grasp_type: str = "active", mode: str = None):
        ...
        if self.grasp_type == "active":
            self.action_scale = [0.015, 0.015, 0.015] + [0.0, 20.0 * DEG_TO_RAD, 0.0] + [20.0 * DEG_TO_RAD] * self.n_finger_joint + [1.0]
...
class NextageShadowRobotCfg(RobotCfg):
    def __init__(self, grasp_type: str = "active", mode: str = None):
        ...
        self.init_joint_pos = {
            ...
            "RARM_JOINT0": 0.0, "RARM_JOINT1": -0.6, "RARM_JOINT2": -0.6, "RARM_JOINT3": 0.0, "RARM_JOINT4": 0.0, "RARM_JOINT5": 0.0,
        ...
        self.actuators = {}
        if "right" in self.eef_keys:
            self.actuators["right_arm"] = ImplicitActuatorCfg(
            ...
            self.actuators["right_hand"] = ImplicitActuatorCfg(
        ...
        if "left" in self.eef_keys:
            self.actuators["left_arm"] = ImplicitActuatorCfg(
            ...
            self.actuators["left_hand"] = ImplicitActuatorCfg(
...
class ShadowRobotCfg(RobotCfg):
    def __init__(self, grasp_type: str = "active", mode: str = None):
        ...
        self.eef_keys = ["right"]
        self.init_joint_pos = {
            "rh_Tx": 0.0, "rh_Ty": 0.0, "rh_Tz": 0.0, "rh_roll": -math.pi/2, "rh_pitch": 0.0, "rh_yaw": -math.pi/2,
        ...
        self.init_pos, self.init_rot = (0.0, 0.0, 1.5), (1.0, 0.0, 0.0, 0.0)
        self.actuators = {
        ...
        self.arm_names = {"right": ["rh_Tx", "rh_Ty", "rh_Tz", "rh_roll", "rh_pitch", "rh_yaw"]}
        self.n_finger_joint = 16

        urdf_path = os.path.abspath(self.usd_path.replace(".usd", ".urdf"))
        self.hand_util = {"right": ShadowHandUtils(grasp_type=grasp_type, hand_laterality="right", urdf_path=urdf_path)}
        self.off_camera_sensor = True
        self.off_contact_sensor = False
        self.camera_data_types = ["rgb", "depth", "semantic_segmentation"] # , "normals"]
        self.compute_pointcloud = True
        if mode == "collect": self.off_camera_sensor = False
        self.camera = CameraCfg(
...
class ShadowRobotMultiCfg(RobotCfg):
    def __init__(self, grasp_type: str = "active", mode: str = None):
        ...
        # set z position to 1.5 to aovid collision with the table
        self.init_pos, self.init_rot = (0.0, 0.0, 1.5), (1.0, 0.0, 0.0, 0.0)
        self.actuators = {
            "right_hand": ImplicitActuatorCfg(
                joint_names_expr=["rh_.*"],
        ...
        self.arm_names = {
            "right": ["rh_Tx", "rh_Ty", "rh_Tz", "rh_roll", "rh_pitch", "rh_yaw"],
            "left": ["lh_Tx", "lh_Ty", "lh_Tz", "lh_roll", "lh_pitch", "lh_yaw"]
        }
        self.n_finger_joint = 16
        urdf_path = os.path.abspath(self.usd_path.replace(".usd", ".urdf"))
        self.hand_util = {
...
def get_robot_cfg(robot_name: str, grasp_type: str = "active", mode: str = None) -> RobotCfg:
    """Get the robot configuration based on the robot name."""
    if robot_name == "nextage-shadow":
        return NextageShadowRobotCfg(grasp_type=grasp_type, mode=mode)
    elif robot_name == "shadow":
        return ShadowRobotCfg(grasp_type=grasp_type, mode=mode)
    elif robot_name == "shadow-multi":
        return ShadowRobotMultiCfg(grasp_type=grasp_type, mode=mode)
    elif robot_name == "ur10-honda":
        return HondaRobotCfg(grasp_type=grasp_type, mode=mode)
    else:
