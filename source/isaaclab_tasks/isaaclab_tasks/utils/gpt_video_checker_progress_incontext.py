...
def build_prompt_content_wo_context(
    ...
    footer = (
        ...
        "(5) The object is detached from the table."
        "Answer with a single number and return a JSON object in the format: {\"answer\": \"1-5\"}"
    )
...
def build_prompt_content(
    ...
    footer = (
        ...
        "(2) The hand is within a few centimeters of the object;"
        "(3) The hand is touching the object;"
        "(4) The hand is securely grasping the object. The object is on the table based on the shading;"
        "(5) The hand is securely grasping the object. The object is detached from the table based on the shading."
        "Note: These images may show multiple environments in the background. "
        "Focus only on the robot and object that appear most prominently in the foreground."
        "How to answer:\n"
        "Return your answer as a JSON object in the following format:\n"
        "{\"answer\": \"1-5\", \"reason\":\"The reason of your choice in less than 10 words.\"}\n"
    )
    content.append({"type": "text", "text": footer})
    return content
...
def gpt_progress_for_frame(frame, demo_image, credpath, wocontext):
    creds = load_credentials(credpath)
    client, client_params = init_vlm_client(creds)
    if wocontext:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Video progress check via GPT-4 Vision.")
    parser.add_argument("--creds", default="auth.env", help="Env file path.")
    parser.add_argument("--num_samples", type=int, default=100, help="Number of random frames to sample.")
    parser.add_argument("--ask", default="grasp_demo_with_shade_testvideo.mp4", help="Test video path.")
    parser.add_argument("--demo_image", default="demo_image_grid_manual.png", help="Demo image path.")
    parser.add_argument("--output_csv", default="progress_vs_time.csv", help="CSV file for saving results.")
    ...
    for idx in indices:
        ...
        if idx == 3:
            scaled_time = 0.0
            out_path = f"frame_time_{scaled_time:.3f}.jpg"
            cv2.imwrite(out_path, frame)
        if total_frames > 1 and idx > 0:
            scaled_time = idx / (total_frames - 1)
            frame_idx_and_data.append((idx, scaled_time, frame))
            # Save if scaled_time is close to any desired time
            for t in desired_times:
                if abs(scaled_time - t) < tolerance:
                    out_path = f"frame_time_{t:.3f}.jpg"
                    cv2.imwrite(out_path, frame)
    ...
    plt.tight_layout()
    if args.wocontext:
        plt.savefig("progress_vs_time_wocontext.png")
    else:
        plt.savefig("progress_vs_time2.png")
