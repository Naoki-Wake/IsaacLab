...
# replace /home/nawake/anaconda3/envs/env_isaaclab/lib/python3.10/site-packages/rl_games/common/experience.py
...
class VectorizedReplayBuffer:
    ...
    def add(self, obs, action, reward, next_obs, done):
        ...
        # print("added reward", self.rewards[self.idx - 1].item(), "to replay buffer at index", self.idx - 1)
    def sample(self, batch_size):
        """Sample a batch of experiences.
        Parameters
        ----------
    ...
    def save_reward_histogram(self, filename="reward_histogram.png", bins=50):
        import matplotlib.pyplot as plt
        import os

        # ヒストグラムを描画
        plt.figure(figsize=(8, 5))
        plt.hist(self.rewards.cpu().numpy().flatten(), bins=bins, edgecolor='black')
        plt.title("Histogram of Rewards in Replay Buffer")
        plt.xlabel("Reward")
...
class VectorizedReplayBufferWithPrioritizedSampling:
    def __init__(self, obs_shape, action_shape, capacity, device):
        """Create Vectorized Replay buffer.
        Parameters
        ----------
    ...
    def add(self, obs, action, reward, next_obs, done):
        ...
        # set priorities as rewards for the sake of simplicity
        priority = reward.clone()
        num_observations = obs.shape[0]
        ...
        for i in range(num_observations):
            if not self.full:
                ...
                # 優先度が最小のインデックスを探す
                min_priority = self.priorities.min()
                min_indices = (self.priorities == min_priority).nonzero(as_tuple=False).flatten()
                idx = min_indices[torch.randint(0, len(min_indices), (1,)).item()]

            # 書き込み
            self.obses[idx] = obs[i]
            self.actions[idx] = action[i]
            self.rewards[idx] = reward[i]
            self.next_obses[idx] = next_obs[i]
            self.dones[idx] = done[i]
            self.priorities[idx] = priority[i]

            # print("Added to idx", idx, "priority", priority[i].item())
    def sample(self, batch_size):
        """Sample a batch of experiences.
        Parameters
        ----------
    ...
    def save_reward_histogram(self, filename="reward_histogram.png", bins=50):
        ...
        plt.figure(figsize=(8, 5))
        plt.hist(self.rewards.cpu().numpy().flatten(), bins=bins, edgecolor='black')
        # set x limits to avoid clipping
        plt.xlim(0, 1.1)
        # set y limits to avoid clipping
        plt.ylim(0, self.capacity)
        plt.title("Histogram of Rewards in Replay Buffer")
        plt.xlabel("Reward")
        plt.ylabel("Frequency")
        plt.grid(True)
...
async def async_ask_gpt(frame):
    ...
    sampled_frames = [cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)]
    return await loop.run_in_executor(None, ask_gpt, client, client_params, sampled_frames)
async def async_ask_phi4(frame):
    sampled_frames = [cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)]
    return await loop.run_in_executor(None, ask_phi4, sampled_frames)
...
class VectorizedReplayBufferGPT:
    def __init__(self, obs_shape, action_shape, capacity, device, images_shape=(480, 640, 3)):
        """Create Vectorized Replay buffer.
        Parameters
        ----------
        size: int
        ...
        self.rewards = torch.empty((capacity, 1), dtype=torch.float32, device=self.device)
        self.dones = torch.empty((capacity, 1), dtype=torch.bool, device=self.device)
        self.checked_by_gpt = torch.zeros((capacity, 1), dtype=torch.bool, device=self.device)
        self.images = torch.empty((capacity, *images_shape), dtype=torch.uint8, device=self.device)
        self.capacity = capacity
        self.idx = 0
        self.full = False
    ...
    def add(self, obs, action, reward, next_obs, done, image):
        ...
        if remaining_capacity < num_observations:
            ...
            self.next_obses[0: overflow] = next_obs[-overflow:]
            self.dones[0: overflow] = done[-overflow:]
            self.images[0: overflow] = image[-overflow:]
            self.full = True
            self.checked_by_gpt[0: overflow] = False
        self.obses[self.idx: self.idx + remaining_capacity] = obs[:remaining_capacity]
        self.actions[self.idx: self.idx + remaining_capacity] = action[:remaining_capacity]
        self.rewards[self.idx: self.idx + remaining_capacity] = reward[:remaining_capacity]
        self.next_obses[self.idx: self.idx + remaining_capacity] = next_obs[:remaining_capacity]
        self.dones[self.idx: self.idx + remaining_capacity] = done[:remaining_capacity]
        self.images[self.idx: self.idx + remaining_capacity] = image[:remaining_capacity]
        self.checked_by_gpt[self.idx: self.idx + remaining_capacity] = False
    ...
    async def update_done_rewards_with_gpt(self, batch_size=8):
        done_mask = self.dones[:self.capacity if self.full else self.idx].squeeze(-1)
        not_checked_mask = ~self.checked_by_gpt[:self.capacity if self.full else self.idx].squeeze(-1)
        target_idxs = torch.nonzero(done_mask & not_checked_mask, as_tuple=False).squeeze(-1)
        #target_idxs = torch.nonzero(not_checked_mask, as_tuple=False).squeeze(-1)
        if len(target_idxs) == 0:
            print("No new done indices to update.")
            return

        #print(f"Updating {len(target_idxs)} rewards via GPT...")

        # バッチ処理
        for i in range(0, len(target_idxs), batch_size):
            batch_idxs = target_idxs[i:i+batch_size]
            frames = self.images[batch_idxs].cpu().numpy()

            # GPTに並列問い合わせ
            #tasks = [async_ask_gpt(frame) for frame in frames]
            tasks = [async_ask_phi4(frame) for frame in frames]         
            results = await asyncio.gather(*tasks)

            # 結果を更新
            for idx_tensor, new_reward in zip(batch_idxs, results):
                idx = idx_tensor.item()
                self.rewards[idx] = float(new_reward)
                self.checked_by_gpt[idx] = True
                #print(f"Updated reward at index {idx}: {float(new_reward)}")
