...
# replace /home/nawake/anaconda3/envs/env_isaaclab/lib/python3.10/site-packages/rl_games/algos_torch/sac_agent.py
class SACAgent(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        self.replay_buffer = experience.VectorizedReplayBufferGPT(self.env_info['observation_space'].shape,
    ...
    def play_steps(self, random_exploration = False):
        ...
        for s in range(self.num_steps_per_episode):
            ...
            # convert infos['frames']['rgb'] to tensor
            images = infos['frames']['rgb']
            if isinstance(images, np.ndarray):
                images = torch.from_numpy(images).to(self._device)
            
            self.replay_buffer.add(obs, action, torch.unsqueeze(rewards, 1), next_obs_processed, torch.unsqueeze(dones, 1), images)
            asyncio.run(self.replay_buffer.update_done_rewards_with_gpt(batch_size=16))  # Automatically update rewards with GPT when buffer is full
    ...
    def train(self):
        ...
        while True:
            ...
            if self.epoch_num % 100 == 0:  # 10エポックごとに書き出し
                self.replay_buffer.save_reward_histogram(
                    filename=os.path.join(self.experiment_dir, f"reward_hist_epoch{self.epoch_num}.png"))
